 ...
 */
function AlertDialog({ isOpen, onClose, onConfirm, title, message, type = 'info', confirmText = 'OK', cancelText = 'Batal', showCancel, hideIcon = false }) {
  ...
  return createPortal(
    <div ref={overlayRef} className={`${styles.overlay} ${styles.alertDebugForce}`} onClick={handleBackdropClick} role="presentation">
      <div className={`${styles.dialog} ${styles[type]}`}>
        {!hideIcon && <div className={styles.iconWrapper}>{getIcon()}</div>}

        {title && <h2 className={styles.title}>{title}</h2>}

        {message && (typeof message === 'string' ? <p className={styles.message}>{message}</p> : <div className={styles.message}>{message}</div>)}
