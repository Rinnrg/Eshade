...
/**
 * Crop an image File to a centered square. Returns a new File instance (JPEG).
 * maxSide: optional maximum width/height for the output to avoid huge images.
 */
export async function cropImageToSquare(file, maxSide = 1600) {
  if (!(file && file.type && file.type.startsWith('image/'))) return file;
  ...
  const destSide = Math.min(side, maxSide);

  // Source coords (center crop)
  ...
  // Draw cropped square scaled to destination size
  ctx.drawImage(img, sx, sy, side, side, 0, 0, destSide, destSide);

  const canvasToBlob = (c, mime, quality) => new Promise((resolve) => {
