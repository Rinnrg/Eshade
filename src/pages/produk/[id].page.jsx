...
function ProdukDetailPage({ produk, error, maintenance }) {
  ...
  if (maintenance && maintenance.active) {
    return (
      <>
        <CustomHead title="Maintenance" description={maintenance.message || 'halaman ini sedang dalam perbaikan'} />
...
export async function getServerSideProps(context) {
  ...
  try {
    // Check maintenance for product detail page
    const detailMaintenance = await prisma.maintenanceSetting.findUnique({ where: { page: 'produk-detail' } });

    if (detailMaintenance && detailMaintenance.active) {
      return {
        props: {
          produk: null,
          error: null,
          maintenance: { active: true, message: detailMaintenance.message || 'halaman ini sedang dalam perbaikan' },
        },
      };
    }
